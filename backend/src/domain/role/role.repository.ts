...
export class RoleRepository implements IRoleRepository {
  async findById(id: string): Promise<RoleWithPermissions | null> {
    const [row] = await query<any>(`${roleWithPermissionsQuery} WHERE r.id = :id`, { id });
    return row ? this.mapRow(row) : null;
  }

  async findByName(name: string): Promise<RoleWithPermissions | null> {
    const [row] = await query<any>(`${roleWithPermissionsQuery} WHERE r.name = :name`, { name });
    return row ? this.mapRow(row) : null;
  }

  async findMany(): Promise<RoleWithPermissions[]> {
    const rows = await query<any>(`${roleWithPermissionsQuery} ORDER BY r.name ASC`);
  ...
  private mapRow(row: any): RoleWithPermissions {
    ...
    return {
      ...
      permissions: permissions.filter((p: any) => p?.id),
    };
  }
}
